...
class CLIP_Loss(nn.Module):
    ...
    def forward(self, image_features, text_features, image_idx=None, text_idx=None):
        ...
        if self.personalized_tau:
            ...
            total_loss = (F.cross_entropy(sim / text_temp, labels) +
                          F.cross_entropy(sim.t() / image_temp, labels)) / 2
        else:
...
class SogCLR_Loss(nn.Module):
    def __init__(self, N=2900000, gamma=0.1, temperature=0.07, world_size=8, bsz=128,
                 enable_surrogate=False, surrogate_c=1.0, lamda_rho=1.0, lamda_init=1.0):
        # Inputs:
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        ...
        # clamp exponentials to avoid inf
        exp_image_diffs = torch.clamp(exp_image_diffs, min=0.0, max=1e4)
        exp_text_diffs  = torch.clamp(exp_text_diffs,  min=0.0, max=1e4)
        ...
        if epoch == 0:
            ...
            # clamp moving averages to avoid 0 or inf
            s_I = s_I.reshape(g_I.shape)
            s_T = s_T.reshape(g_T.shape)

        s_I = s_I.clamp(min=self.eps, max=1e4)
        s_T = s_T.clamp(min=self.eps, max=1e4)
        ...
        # final safety clamp
        weights_image = torch.nan_to_num(weights_image, nan=0.0, posinf=0.0, neginf=0.0)
        weights_text  = torch.nan_to_num(weights_text,  nan=0.0, posinf=0.0, neginf=0.0)
...
class iSogCLR_New_Loss(nn.Module):
    def __init__(self, N=2900000, gamma=0.8, tau_init=0.01, world_size=8, bsz=128,
        ...
        if self.use_temp_net:
            self.image_temp_gen = TempGenerator(feature_dim=feature_dim, M=256,
                                                tau_min=self.tau_min, tau_max=self.tau_max).cuda()
            self.text_temp_gen = TempGenerator(feature_dim=feature_dim, M=256,
                                               tau_min=self.tau_min, tau_max=self.tau_max).cuda()
        else:
            self.beta_u = 0.5
            self.grad_clip = 5.0
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch, max_epoch):
        ...
        if self.use_temp_net:
            ...
            self.u_I[image_ids] = ((1.0 - self.beta_u) * self.u_I[image_ids] +
                                   self.beta_u * temp_weight_image.squeeze().clamp_(min=-self.grad_clip, max=self.grad_clip))
            self.u_T[text_ids] = ((1.0 - self.beta_u) * self.u_T[text_ids] +
                                   self.beta_u * temp_weight_text.squeeze().clamp_(min=-self.grad_clip, max=self.grad_clip))
        ...
        return (total_loss,
                ...
                temp_weight_image.max().item(),
                temp_weight_image.min().item())
...
class CyCLIP_Loss(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        contrastive_loss = (self.criterion(logits_text_per_image, target) +
                            self.criterion(logits_image_per_text, target)) / 2.0
...
class VICReg_Loss(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        cov_loss = (off_diagonal(cov_x).pow_(2).sum().div(self.dim_size) +
                    off_diagonal(cov_y).pow_(2).sum().div(self.dim_size))
...
class iSogCLR_New_v1_Loss(nn.Module):
    def __init__(self, N=2900000, gamma=0.8, tau_min=0.005, tau_max=1.0,
                 rho_init=6.0, bsz=128, world_size=8, eps=1e-6, M=64, feature_dim=256):
        # Inputs:
        #   N is the number of samples in training set
        #   M is number of prototypes in the temperature generator
        ...
        self.image_temp_gen = TempGenerator(feature_dim=feature_dim, M=M,
                                            tau_min=tau_min, tau_max=tau_max).cuda()
        self.text_temp_gen = TempGenerator(feature_dim=feature_dim, M=M,
                                           tau_min=tau_min, tau_max=tau_max).cuda()
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        ...
        temp_weight_image = (torch.log(s_I) +
                             self.b_I[image_ids][:, None] +
                             self.rho_I -
                             torch.sum(weights_image * image_temp_term, dim=1, keepdim=True) / self.neg_num)
        temp_weight_text = (torch.log(s_T) +
                            self.b_T[text_ids][None, :] +
                            self.rho_T -
                            torch.sum(weights_text * text_temp_term, dim=0, keepdim=True) / self.neg_num)
...
class iSogCLR_New_v2_Loss(nn.Module):
    ...
    def forward(self, image_features, text_features, image_ids, text_ids, epoch, max_epoch):
        ...
        if epoch == 0:
            ...
            r = ((1.0 - self.gamma) * self.r * torch.exp(old_b_pos - self.b_pos) +
                 self.gamma * torch.mean(exp_pos_temp_term))
        ...
        grad_tau_image = (torch.log(s_I) +
                          self.b_I[image_ids][:, None] +
                          self.rho_I -
                          torch.sum(weights_image * image_temp_term, dim=1, keepdim=True) / (batch_size - 1))
        grad_tau_text = (torch.log(s_T) +
                         self.b_T[text_ids][None, :] +
                         self.rho_T -
                         torch.sum(weights_text * text_temp_term, dim=0, keepdim=True) / (batch_size - 1))
        ...
        return (total_loss,
                ...
                avg_text_tau,
                self.eta_init,
                grad_tau_image.mean().item(),
                grad_tau_text.mean().item(),
                old_b_I.mean().item(),
                old_b_T.mean().item(),
                0.0,
                0.0)
...
class onlineCLR_Loss(nn.Module):
    ...
    def forward(self, image_features, text_features):
        ...
        self.u_n = ((1 - self.gamma) * self.u_n.cuda() * torch.exp((self.c_n - max_neg_logits) / self.nT) +
                    self.gamma * torch.sum(neg_logits1_exp + neg_logits2_exp).detach())
        self.u_p = ((1 - self.gamma) * self.u_p.cuda() * torch.exp((self.c_p - max_pos_logits) / self.pT) +
        ...
        def softmax_cross_entropy_with_logits_v2(pos_logits, pos_weights, neg_logits, neg_weights):
            expsum_neg_logits = torch.sum(neg_weights * neg_logits)
            expsum_pos_logits = torch.sum(pos_weights * pos_logits)
...
class HardNegCLIP_Loss(nn.Module):
    """
    Two-way CLIP-style loss with hard-negative reweighting.

    - Uses standard image<->text similarities.
    - Up-weights hard negatives (negatives that are close to the positive).
    - Symmetric image->text and text->image, like CLIP.
    - Compatible with distributed training via GatherLayer.
    """

    def __init__(self, world_size=8, temperature=0.01, hard_weight=2.0, margin=0.0, eps=1e-12):
        super(HardNegCLIP_Loss, self).__init__()
        self.world_size = world_size
        self.temperature = temperature
    ...
    def forward(self, image_features, text_features):
        ...
        logits = torch.einsum("i d, j d -> i j", image_features, text_features) / self.temperature
        logits_t = logits.t()
        targets = torch.arange(batch_size, device=device)

        mask = torch.ones_like(logits, dtype=torch.bool, device=device)
        mask[range(batch_size), range(batch_size)] = False

        with torch.no_grad():
            neg_sims_i = logits.masked_fill(~mask, float("-inf"))
            pos_sims_i = logits[range(batch_size), range(batch_size)]
            hard_mask_i = (neg_sims_i + self.margin >= pos_sims_i.view(-1, 1)).float()
            weight_i = 1.0 + self.hard_weight * hard_mask_i

        logits_i = logits - logits.max(dim=1, keepdim=True)[0]
        exp_i = torch.exp(logits_i) * weight_i
        log_prob_i = logits_i - torch.log(exp_i.sum(dim=1, keepdim=True) + self.eps)
        loss_i = -log_prob_i[range(batch_size), targets].mean()

        with torch.no_grad():
            neg_sims_t = logits_t.masked_fill(~mask, float("-inf"))
            pos_sims_t = logits_t[range(batch_size), range(batch_size)]
            hard_mask_t = (neg_sims_t + self.margin >= pos_sims_t.view(-1, 1)).float()
            weight_t = 1.0 + self.hard_weight * hard_mask_t

        logits_t2 = logits_t - logits_t.max(dim=1, keepdim=True)[0]
        exp_t = torch.exp(logits_t2) * weight_t
        log_prob_t = logits_t2 - torch.log(exp_t.sum(dim=1, keepdim=True) + self.eps)
        loss_t = -log_prob_t[range(batch_size), targets].mean()

        loss = 0.5 * (loss_i + loss_t)
        return loss


class MarginHardNegCLIP_Loss(nn.Module):
    def __init__(self, world_size=8, temperature=0.01, margin=0.2, alpha=0.5):
        super(MarginHardNegCLIP_Loss, self).__init__()
        self.world_size = world_size
        self.temperature = temperature
        self.margin = margin   # desired gap between pos and hardest neg
        self.alpha = alpha     # weight on margin term
    ...
    def forward(self, image_features, text_features):
        ...
        # similarity matrix
        sims = torch.einsum("i d, j d -> i j", image_features, text_features)
        ...
        clip_loss = (
            ...
            F.cross_entropy(logits_t, targets)
        ) / 2.0
        ...
        pos_sims = sims[range(b), range(b)]  # [B]

        # image->text hardest negative
        neg_sims_i = sims.masked_fill(~mask, float("-inf"))
        hardest_neg_i, _ = neg_sims_i.max(dim=1)  # [B]
        margin_loss_i = F.relu(self.margin - (pos_sims - hardest_neg_i))

        # text->image hardest negative
        neg_sims_t = sims.t().masked_fill(~mask, float("-inf"))
        hardest_neg_t, _ = neg_sims_t.max(dim=1)  # [B]
        margin_loss_t = F.relu(self.margin - (pos_sims - hardest_neg_t))

        margin_loss = (margin_loss_i.mean() + margin_loss_t.mean()) / 2.0

        return clip_loss + self.alpha * margin_loss

class SogCLR_Margin_Loss(SogCLR_Loss):
    """
    SogCLR + positive margin term.
    Inherits all SogCLR machinery, just adds a hinge on positive sims.
    """

    def __init__(self, margin=0.2, margin_weight=0.1, **kwargs):
        super().__init__(**kwargs)
        self.margin = margin
        self.margin_weight = margin_weight

    def forward(self, image_features, text_features, image_ids, text_ids, epoch):
        base_loss, _, _ = super().forward(image_features, text_features, image_ids, text_ids, epoch)

        # recompute sim on current batch (they're small, so OK)
        if self.world_size > 1:
            image_features = torch.cat(GatherLayer.apply(image_features), dim=0)
            text_features = torch.cat(GatherLayer.apply(text_features), dim=0)

        sim = torch.einsum('i d, j d -> i j', image_features, text_features)
        diag_sim = torch.diagonal(sim)  # [B]

        # hinge: want diag_sim >= margin
        pos_margin_loss = F.relu(self.margin - diag_sim).mean()

        total_loss = base_loss + self.margin_weight * pos_margin_loss
        return total_loss, 0.0, 0.0
